...
export const resetPassword = async (req, res, next) => {
  try {
    // 1️⃣ Extract validated fields from the Zod middleware
    const { token } = req.validated.params;
    const { password } = req.validated.body;

    // 2️⃣ Hash the token to match what's stored in DB
    const hashedToken = crypto
      .createHash('sha256')
      .update(token)
      .digest('hex');

    // 3️⃣ Find user by token and check expiration
    ...
    // 4️⃣ Update password & clear reset token and expiration
    ...
    // 5️⃣ Log the user in automatically
